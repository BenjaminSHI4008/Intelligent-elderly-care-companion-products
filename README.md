...
### Clive-Elder-v0.5 — 2026-07-02（老人端 AI 聊天体验升级）

**新聊天窗口**
- AI 聊天页右上角新增「新聊天」入口，可一键开启空白新会话
- 聊天区中间新增「新的聊天」引导卡片，提示老人上方的历史记录和新增对话功能
- 参考 Codex 体验：未输入内容的空新会话不会保存到历史记录
- AI 回复中禁止新建或切换会话，避免异步回复写入错误聊天

**手写/键盘输入**
- AI 聊天页新增「✍ 手写/键盘输入 / 🎤 语音输入」切换入口，支持在语音按钮和文字输入框之间切换
- 文字输入模式调用系统键盘，用户可通过手机输入法切换到手写输入
- 输入模式会记住上次选择，文字消息发送后继续复用现有 AI 回复与语音播报流程

**历史记录管理**
- AI 聊天历史侧边栏每条记录新增红色圆形删除按钮
- 删除历史前新增确认弹窗，避免误触后直接删除
- 删除当前正在查看的历史会话后，主聊天区自动回到空的新聊天状态
- 历史记录行和删除按钮新增按下变浅色反馈，点击状态更清晰

**空态轮播与手势**
- 中间空态卡片新增「新的聊天 / 聊天记录」双页介绍
- 空态介绍每 5 秒自动左滑切换，并在底部显示两个半透明页码点
- 支持左右拖动空态卡片内容，拖过阈值后手动切换介绍页
- 发送消息或加载历史后，空态轮播与拖动状态自动停止

---

### Clive v0.1 — 2026-07-01（老人端 UI 与 AI 历史体验）

**老人端 UI 优化**
- 注册页新增左上角返回按钮，返回登录页更直观
- 老人端返回按钮统一调整为绿色系，贴合「重新生成」按钮的主色风格
- 首页麦克风按钮去除底部非圆形阴影，并放大「按住说话」文字
- AI 聊天页右上角历史入口按钮放大，提升老人用户可点击性

**AI 聊天历史**
- AI 聊天页新增右侧历史侧边栏，可查看本机历史对话
- 历史会话存储在本地，按最近更新时间排序
- 会话标题根据首次对话内容自动生成，方便快速识别历史主题
- 点击历史会话可恢复对应问答内容

**麦克风新会话跳转**
- 首页按住说话识别成功后自动进入 AI 聊天页
- 每次首页语音对话都会生成新的 `sessionId`，作为一条新对话保存
- 空语音、识别失败或网络失败不会创建历史会话
- 首页「AI聊天记录」入口不再默认注入示例对话
